...
" TODO: vim-endwise fights with Coc completions and pumvisible.
...
Plug 'romainl/vim-cool'     " Turn off hlsearch automagically
...
Plug 'AndrewRadev/bufferize.vim'  " Bufferize to push a vim command's output into a temporary buffer
...
Plug 'iamcco/markdown-preview.nvim', {'do': { -> mkdp#util#install() }}  " markdown preview plugin
...
Plug 'sirtaj/vim-openscad'            " OpenSCAD 3D CAD
...
if has('macunix')
  " focus next tab
  noremap <S-D-}> <Cmd>tabnext<CR>           " focus next tab
...
" Split movement
nnoremap <silent><space>wk <C-w>K  " current window to top
nnoremap <silent><space>wj <C-w>J  " current window to bottom
nnoremap <silent><space>wh <C-w>H  " current window to left
nnoremap <silent><space>wl <C-w>L  " current window to right
nnoremap <silent><space>wt <C-w>T  " current window to new tab
...
nmap <silent><space>shh <Cmd>CocList files ~<CR>
nmap <silent><space>shc <Cmd>CocList files ~/code<CR>
nmap <silent><space>shs <Cmd>CocList files ~/source<CR>
nmap <silent><space>sca <Cmd>CocList files ~/Documents/CAD<CR>
" filetypes
